...
function marcarTareaCompletada(){
    if(tareas.length === 0){
        ...
        let task = parseInt(prompt("Ingresa un numero de tarea" + mensaje));

        if(task > 0 && task <= tareas.length){
            tareas[task-1].completada = true;
            alert(`La tarea "${tareas[task-1].nombre}" se ha marcado como completada!`);
        }else{
            alert("Escoge bien w");
        }
